...
namespace toolkit
{
	...
	{
		return toolkit::static_fnv32< LENGTH >(chars);
	}
	...
	{
		return toolkit::static_fnv64< LENGTH >(chars);
	}
	...
	{
		uint32_t hash = toolkit::fnv_basis_32;

		for (size_t index = 0; index < LENGTH; ++index)
		{
			hash ^= *(chars++); //da error aqui
			hash *= toolkit::fnv_prime_32;
		}
